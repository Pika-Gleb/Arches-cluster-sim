...
def determine_remnant_for_massive_star(initial_mass_msun, metallicity=0.008, stellar_type=""):
    ...
    if initial_mass_msun < 8.0:
        ...
        return ("White Dwarf", remnant_mass, "WD",
                f"Белый карлик. Исходная масса {initial_mass_msun:.1f} M☉, Z={metallicity:.4f}.")
        ...
        return ("NO_REMNANT", 0.0, "PISN",
                f"Парно-нестабильная сверхновая. Полное разрушение {initial_mass_msun:.0f} M☉ звезды (Z={metallicity:.4f}).")
        ...
        return ("Black Hole", remnant_mass, "BH",
                f"Чёрная дыра после пульсаций и сброса оболочки. Исходная масса {initial_mass_msun:.1f} M☉.")
        ...
        return ("Black Hole", remnant_mass, "BH",
                f"Прямой коллапс в чёрную дыру. Исходная масса {initial_mass_msun:.1f} M☉, сохранено {mass_fraction * 100:.0f}%.")
    else:
        mass_fraction = np.random.uniform(0.2, 0.35)
        remnant_mass = initial_mass_msun * mass_fraction * z_factor
        remnant_mass = max(OPPENHEIMER_VOLKOFF_LIMIT * 1.2, remnant_mass)
        return ("Black Hole", remnant_mass, "BH",
                f"Чёрная дыра (через сверхновую). Исходная масса {initial_mass_msun:.1f} M☉.")
    ...
    for star in stars:
        ...
        if lifetime_myr > 0 and current_age_myr >= lifetime_myr * MAX_AGE_FACTOR:
            ...
            if remnant_type == "NO_REMNANT":
                print(
                    f"[ПАРНО-НЕСТАБИЛЬНАЯ СВЕРХНОВАЯ] Время {current_time.value_in(units.Myr):.3f} Myr: Звезда {star.initial_mass:.1f} M☉ ({stellar_type}) полностью разрушена без остатка.")
            ...
            print(
                f"[КОЛЛАПС] Время {current_time.value_in(units.Myr):.3f} Myr: Звезда {star.initial_mass:.1f} M☉ ({stellar_type}, возраст {current_age_myr:.1f}/{lifetime_myr:.1f} Myr) -> {remnant_type} {remnant_mass_val:.2f} M☉ (потеря {mass_loss_percent:.1f}% массы)")
            stars_to_remove.add_particle(star)
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 70)
    print("ЗАПУСК КАСКАДА ИЗ 7 ЦИКЛОВ")
    print("=" * 70)
